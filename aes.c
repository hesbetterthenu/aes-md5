#include "aes.h"
#include "md5.h"
#include <string.h>
#include <stdint.h>
#include <malloc.h>
#define RIGHTROTATE32(x, c) (((x) << (c)) | ((x) >> (32 - (c))))
#define LEFTROTATE32(x, c) (((x) >> (c)) | ((x) << (32 - (c))))
#define Multiply(x, y) (((y & 1) * x) ^ ((y >> 1 & 1) * xtime(x)) ^ ((y >> 2 & 1) * xtime(xtime(x))) ^ ((y >> 3 & 1) * xtime(xtime(xtime(x)))) ^ ((y >> 4 & 1) * xtime(xtime(xtime(xtime(x))))))
#define xtime(x) ((x << 1) ^ (((x >> 7) & 1) * 0x1b))
static const unsigned char sbox[] = {
		
};
static const unsigned char isbox[] = {
		
};
static const uint32_t rcon[] = { 0x00000000, 0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000 };
#define exk_l 176
#define rel_l 16
#define nr    10
void scipherD(char* a, char* key, int k) // a%16 = 0
{
	unsigned char exkey[exk_l];
	unsigned char realkey[rel_l];
	md5((unsigned char*)key, strlen(key), realkey);
	keyex(realkey, exkey);
	char* temp = a;
	for (int i = 0; i < k; i++)
	{
		cipherit(temp, exkey);
		temp += 16;
	}
}
void invscipherD(char* a, char* key, int k)
{
	unsigned char exkey[exk_l];
	unsigned char realkey[rel_l];
	md5((unsigned char*)key, strlen(key), realkey);
	keyex(realkey, exkey);
	char* temp = a;
	for (int i = 0; i < k; i++)
	{
		invcipherit(temp, exkey);
		temp += 16;
	}
}

void scipher(char* a, char* key, char** res)
{
	size_t k = strlen(a);
	size_t k1 = k % 16;
	k = k / 16;
	*res = (char*) malloc ((k+1) * 16);
	memcpy(*res, a, strlen(a));
	memset(*res + k * 16 + k1, 0, 16 - k);
	k++;
	unsigned char exkey[exk_l];
	unsigned char realkey[rel_l];
	md5((unsigned char*)key, strlen(key), realkey);
	keyex(realkey, exkey);
	char* temp = *res;
	for (int i = 0; i < k; i++)
	{
		cipherit(temp, exkey);
		temp += 16;
	}
}
void invscipher(char* a, char* key, char** res)
{
	size_t k = strlen(a);
	size_t k1 = k % 16;
	k = k / 16;
	*res = (char*) malloc ((k+1) * 16);
	memcpy(*res, a, strlen(a));
	memset(*res + k * 16 + k1, 0, 16 - k);
	k++;

	unsigned char exkey[exk_l];
	unsigned char realkey[rel_l];
	md5((unsigned char*)key, strlen(key), realkey);
	keyex(realkey, exkey);
	char* temp = *res;
	for (int i = 0; i < k; i++)
	{
		invcipherit(temp, exkey);
		temp += 16;
	}
}
void cipherit(char* a, unsigned char* exkey)
{
	unsigned char state[16];
	memcpy(state, a, 16);
	addroundkey(state, exkey);
	unsigned char* temp = exkey;
	temp += 16;
	for (int i = 1; i < nr; i++)
	{
		subbytes(state);
		shiftrows(state);
		mixcolumns(state);
		addroundkey(state, temp);
		temp += 16;
	}
	subbytes(state);
	shiftrows(state);
	addroundkey(state, temp);
	memcpy(a, state, 16);

}
void invcipherit(char* a, unsigned char* exkey)
{
	unsigned char state[16];
	memcpy(state, a, 16);
	unsigned char* temp = exkey + 16 * nr;
	addroundkey(state, temp);
	temp -= 16;
	for (int i = 1; i < nr; i++)
	{
		invshiftrows(state);
		invsubbytes(state);
		addroundkey(state, temp);
		temp -= 16;
		invmixcolumns(state);
	}
	invshiftrows(state);
	invsubbytes(state);
	addroundkey(state, temp);
	memcpy(a, state, 16);

}
void cipher(char* a, char* key, char* res)
{
	unsigned char exkey[exk_l];
	unsigned char realkey[rel_l];
	unsigned char state[16];
	md5((unsigned char*)key, strlen(key), realkey);
	keyex(realkey, exkey);
	memcpy(state, a, 16);
	addroundkey(state, exkey);
	unsigned char* temp = exkey;
	temp += 16;
	for (int i = 1; i < nr; i++)
	{
		subbytes(state);
		shiftrows(state);
		mixcolumns(state);
		addroundkey(state, temp);
		temp += 16;
	}
	subbytes(state);
	shiftrows(state);
	addroundkey(state, temp);
	memcpy(res, state, 16);
}
void invcipher(char* a, char* key, char* res)
{
	unsigned char exkey[exk_l];
	unsigned char realkey[rel_l];
	unsigned char state[16];
	md5((unsigned char*)key, strlen(key), realkey);
	keyex(realkey, exkey);
	memcpy(state, a, 16);
	unsigned char* temp = exkey + 16*nr;
	addroundkey(state, temp);
	temp -= 16;
	for (int i = 1; i < nr; i++)
	{
		invshiftrows(state);
		invsubbytes(state);
		addroundkey(state, temp);
		temp -= 16;
		invmixcolumns(state);
	}
	invshiftrows(state);
	invsubbytes(state);
	addroundkey(state, temp);
	memcpy(res, state, 16);
}
void subbytes(unsigned char* state)
{
	for (int i = 0; i < 16; i++)
		state[i] = sbox[state[i]];
	return;
}
void invsubbytes(unsigned char* state)
{
	for (int i = 0; i < 16; i++)
		state[i] = isbox[state[i]];
	return;
}
void shiftrows(unsigned char* state)
{
	uint32_t* temp = (uint32_t*)state;
	for (int i = 1; i < 4; i++)
		temp[i] = LEFTROTATE32(temp[i], i*8);
	return;
}
void invshiftrows(unsigned char* state)
{
	uint32_t* temp = (uint32_t*)state;
	for (int i = 1; i < 4; i++)
		temp[i] = RIGHTROTATE32(temp[i], i*8);
	return;
}
void mixcolumns(unsigned char* state)
{
	unsigned char i;
	unsigned char Tmp, Tm, t;
	for (i = 0; i < 4; ++i)
	{
		t = state[i*4+0];
		Tmp = state[i * 4 + 0] ^ state[i * 4 + 1] ^ state[i * 4 + 2] ^ state[i * 4 + 3];
		Tm = state[i * 4 + 0] ^ state[i * 4 + 1]; 
		Tm = xtime(Tm);  
		state[i * 4 + 0] ^= Tm ^ Tmp;
		Tm = state[i * 4 + 1] ^ state[i * 4 + 2]; 
		Tm = xtime(Tm);  
		state[i * 4 + 1] ^= Tm ^ Tmp;
		Tm = state[i * 4 + 2] ^ state[i * 4 + 3]; 
		Tm = xtime(Tm);  
		state[i * 4 + 2] ^= Tm ^ Tmp;
		Tm = state[i * 4 + 3] ^ t;              
		Tm = xtime(Tm);  
		state[i * 4 + 3] ^= Tm ^ Tmp;
	}
}
void invmixcolumns(unsigned char* state)
{
	int i;
	unsigned char a, b, c, d;
	for (i = 0; i < 4; ++i)
	{
		a = state[i * 4 + 0];
		b = state[i * 4 + 1];
		c = state[i * 4 + 2];
		d = state[i * 4 + 3];
		state[i * 4 + 0] = Multiply(a, 0x0e) ^ Multiply(b, 0x0b) ^ Multiply(c, 0x0d) ^ Multiply(d, 0x09);
		state[i * 4 + 1] = Multiply(a, 0x09) ^ Multiply(b, 0x0e) ^ Multiply(c, 0x0b) ^ Multiply(d, 0x0d);
		state[i * 4 + 2] = Multiply(a, 0x0d) ^ Multiply(b, 0x09) ^ Multiply(c, 0x0e) ^ Multiply(d, 0x0b);
		state[i * 4 + 3] = Multiply(a, 0x0b) ^ Multiply(b, 0x0d) ^ Multiply(c, 0x09) ^ Multiply(d, 0x0e);
	}
}
void addroundkey(unsigned char* state, unsigned char* rkey)
{
	for (int i = 0; i < 4; i++)
		*((uint32_t*)state + i) ^= *((uint32_t*)rkey + i);
}
void keyex(unsigned char* key, unsigned char* exkey)
{
	memcpy(exkey, key, 16);
	for (int i = 0; i < 4; i++)
		*((uint32_t*)exkey + i) = *((uint32_t*)key + i);
	uint32_t temp;
	for (int i = 4; i < 44; i++)
	{
		temp = *((uint32_t*)exkey + i - 1);
		if (i % 4 == 0)
		{
			temp = LEFTROTATE32(temp, 1*8);
			temp  ^= rcon[i / 4];
		}
		*((uint32_t*)exkey + i) = *((uint32_t*)exkey + i - 4) ^ temp;
	}
}
